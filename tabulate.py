...
def create_table(small_count, small_avg_size, small_avg_time, small_total_time, large_count, large_avg_size,
    ...
    table.add_row(['100KB and under', small_count, f"{small_avg_size / 1024:.2f}", f"{small_avg_time:.2f}",
                   f"{small_total_time:.2f}"])

    # Add large files data
    table.add_row(
        ['1-5MB', large_count, f"{large_avg_size / 1024:.2f}", f"{large_avg_time:.2f}", f"{large_total_time:.2f}"])
